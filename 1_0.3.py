...
def Tabu_search(init_solution, tabu_tenure, CC, first_time, Data1, index_consider_elite_set, start_time):
    ...
    with open('Random_'+str(data_set)+'_'+str(number_of_cities)+'_'+str(delta)+'_'+str(alpha)+'_'+str(END_SEGMENT)+'_CL1.txt', 'a') as file:  # Open a file in append mode
        while T < SEGMENT:
            end_time = time.time()
            if end_time - start_time > TIME_LIMIT:
                file.write(f"Breaking out of loop: current_sol={best_sol}, current_fitness={best_fitness}, segment={T}, weight={weight}\n")
                break
